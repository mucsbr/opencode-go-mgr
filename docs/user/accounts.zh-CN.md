...
- **GOAT 推理冷却**：真实 Command Code `429` 若明确指出 5 小时或周套餐窗口，则使用响应中的精确
  `Your limit resets at` 时间写入对应账号冷却。普通瞬时限流或畸形响应仍使用通用 5 分钟兜底。Command Code
  也可能在月度 credits 已不足以支付下一次请求时返回结构化 `400`；只有该精确的 credits 不足响应会切换账号，并按
  已保存购买日期的自然月续期时间写入月冷却，其他 `400` 仍作为普通请求错误处理。
